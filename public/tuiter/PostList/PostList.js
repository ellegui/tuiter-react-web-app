const Post = () => {
    return (`
    <div>
        <ul class="list-group" style = "text-align: left">
            <li class="list-group-item ">
            <img class="wd-pic-right" 
                 src="https://i.guim.co.uk/img/media/3f6d39f213b18361c95ad0d4672ebf5680d19e7d/0_19_3500_2100/master/3500.jpg?width=1200&height=1200&quality=85&auto=format&fit=crop&s=eb570dd786c1a370ca16a41696279c26" 
                 alt="Avatar"/>Elon Musk <img src="../explore/images/verify.png" class="wd-icon-verify"> <span class="wd-lightgray">@elonmusk</span> 
                 <img src="../explore/images/dots.png" class="wd-icon-verify wd-right">
            <div class = "wd-left-text" style="padding-left: 70px">Amazing show about <a href="https://en.wikipedia.org/wiki/Inspiration4">@inspireation4x</a></><div>
            <div class="wd-padding-5"></div>
            <img class="wd-home-mid-pic"
            src="https://occ-0-58-993.1.nflxso.net/dnm/api/v6/E8vDc_W8CLv7-yMQu8KMEC7Rrr8/AAAABfCf8CWNH7SYaPjyY2E8MTCpUNP39NZxcYUgF2Rn9lUy1M8oacnNhGmL1X74qR7y7l8fYjsEnf5ptP2ScpS6Q9MQ-tpYolYcf0Fs.jpg?r=3d5"/>
            <div class="wd-padding-5"></div>
            <div class="wd-home-title"> Countdown: Inspiration4 Mission to Space | Netflix Official Site</div>
            <div class="wd-home-content">From training to launch to landing, this all-access docuseries rides along with the Inspiration4 crew on the first all-civilian orbital space </div>
            <div class="wd-home-link"> <img src="../explore/images/link.png" class="wd-icon wd-bg-trans"> 
            netflix.com </div> 
            <div class="wd-padding-5"></div>
            
            <div class="d-flex justify-content-between ms-2 mb-1">
                <div class="text-secondary float-start">
                <i class="far fa-comment text-secondary"></i> 4.2k
                </div>
             
                <div class="text-secondary float-start">
                <i class="far fa-square text-secondary"></i> 3.2k
                </div>
                
                <div class="text-secondary float-start">
                <i class="far fa-heart text-secondary"></i> 37.5k
                </div>
                
                <div class="text-secondary float-start">
                <i class="far fa-share-square text-secondary"></i>
                </div>
             </div>
             </li>
        </ul>
      </div>
      
      <div class="wd-section-line"></div>
      
      <div>
        <ul class="list-group" style = "text-align: left">
            <li class="list-group-item">
            <img class="wd-pic-right" 
                 src="data:image/png;base64," 
                 alt="Avatar"/> New York Post <img src="../explore/images/verify.png" class="wd-icon-verify"> <span class="wd-lightgray">@nypost</span> 
                 <img src="../explore/images/dots.png" class="wd-icon-verify wd-right">
            <div class = wd-left-text style="padding-left: 70px">Grimes seen reading Karl Marx following split with world’s richest man Elon Musk 
            <a href="https://nypost.com/2021/10/02/grimes-reads-karl-marx-after-split-with-worlds-richest-man-elon-musk/">al.home/fdhoeirh</a> </><div>
            <div class="wd-padding-5"></div>
            <img class="wd-home-mid-pic"
            src="https://nypost.com/wp-content/uploads/sites/2/2021/10/grimes-05.jpg?quality=90&strip=all"/>
            <div class="wd-padding-5"></div>

            <div class="d-flex justify-content-between ms-2 mb-1">
                <div class="text-secondary float-start">
                <i class="far fa-comment text-secondary"></i> 4.2k
                </div>
             
                <div class="text-secondary float-start">
                <i class="far fa-square text-secondary"></i> 3.2k
                </div>
                
                <div class="text-secondary float-start">
                <i class="far fa-heart text-secondary"></i> 37.5k
                </div>
                
                <div class="text-secondary float-start">
                <i class="far fa-share-square text-secondary"></i>
                </div>
             </div>
             </li>
        </ul>
      </div>    
           
  `)
}
export default Post;